...
/* ────────────────────────────────────────────────────────────────────────────
   GradientWaves — campo de ondas em raymarching, no fundo da tela de acesso.

   Do React Bits. Um triângulo de tela cheia com um shader que marcha raios
   contra um campo de senos; a cor vem da distância (perto = crista, longe =
   horizonte) e o alfa desvanece com a névoa, então o efeito se funde com o
   fundo da página em vez de recortá-lo.

   Ele já é bem comportado por conta própria — o laço só roda com o contêiner na
   tela E a aba visível (IntersectionObserver + visibilitychange). Um desvio
   nosso: RESPEITA "reduzir movimento", desenhando um quadro parado em vez de
   animar. O original anima sempre.
   ──────────────────────────────────────────────────────────────────────────── */
...
const detailToSteps = (d) => (d === "low" ? 40.0 : d === "high" ? 110.0 : 70.0);
...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    /* Com movimento reduzido, um quadro só: a cena existe, parada. */
    const tryStart = () => {
      if (reduzido) { renderer.render({ scene: mesh }); return; }
  ...
  useEffect(() => {
    ...
    const aplicar = (alvo, hex) => { const c = hexToRgb(hex); alvo[0] = c[0]; alvo[1] = c[1]; alvo[2] = c[2]; };
    aplicar(u.uHorizonColor.value, horizonColor);
    aplicar(u.uWaveColor.value, waveColor);
    aplicar(u.uCrestColor.value, crestColor);
  }, [horizonColor, waveColor, crestColor, speed, amplitude, waveScale, waveRatio, swell,
      turbulence, tilt, zoom, height, fogDepth, detail, brightness, opacity, grain,
