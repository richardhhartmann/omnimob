...
/* ────────────────────────────────────────────────────────────────────────────
   GhostCursor — rastro de fumaça que segue o ponteiro, em WebGL.

   Do React Bits. Uma malha de tela cheia com um shader de ruído (fbm); o rastro
   é um anel de posições anteriores passado ao shader, e cada uma desenha um
   borrão com peso decrescente. Depois vêm bloom e grão.

   O componente já é econômico por conta própria — o laço PARA quando o rastro
   termina de desvanecer, e só volta ao próximo movimento. Quem usa ainda deve
   montá-lo apenas quando a seção estiver à vista: um contexto WebGL parado
   continua ocupando memória de vídeo, e esta página já tem outro (a névoa do
   Vanta, na seção final).
   ──────────────────────────────────────────────────────────────────────────── */
...
const GhostCursor = ({
  ...
  useEffect(() => {
    ...
    /* Com "reduzir movimento" o efeito não entra: ele é rastro em movimento
       constante, e não há versão estática dele que faça sentido. */
    ...
    resize();
    /* Segunda medição no quadro seguinte.

       A primeira roda no efeito, e há casos em que a caixa ainda não tem
       tamanho ali — o host é absoluto dentro de outro absoluto, e o canvas
       ficava nos 300px padrão do elemento, com `hasValidSize` falso: o laço
       rodava sem nunca desenhar nada. O ResizeObserver cobre a maioria dos
       casos, mas não dispara quando o tamanho já era o final desde o começo. */
    const primeiroQuadro = requestAnimationFrame(() => { if (active) resize(); });
    ...
    const animate = () => {
      ...
      for (let i = 0; i < N; i += 1) arr[i].copy(trailBufRef.current[(headRef.current - i + N) % N]);
      ...
      // Rastro apagado e ponteiro fora: o laço PARA. Volta no próximo gesto.
    ...
    return () => {
      ...
      renderer.dispose();
      /* `forceContextLoss` é o que devolve o contexto WebGL de verdade. Sem ele
         o navegador segura um número pequeno de contextos e, ao entrar e sair da
         seção algumas vezes, o mais antigo é derrubado — e é a névoa do Vanta,
         noutra seção, que apaga sem explicação. */
      renderer.forceContextLoss();
      rendererRef.current = null;

      /* `remove()` e não `parentElement.removeChild()`: o segundo lança
         NotFoundError quando o canvas já saiu do DOM — e ele sai, porque
         `forceContextLoss` e a desmontagem do React competem pela mesma
         limpeza. `remove()` num nó já solto não faz nada. */
